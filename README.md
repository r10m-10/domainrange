...
2. **Parser:** This is the main step in the ast generation process. It is full of recursion so i once again recommend everone to check my hand written [recursion test cases](tests) (1, 2 and 3) because if you understand what I have done there then you would get a better idea about the algorithm than I could explain you in words. This is called Recursive Descent Parsing. In the parsing algorithm, we are basically making a chain of functions that call each other in each pass and track the position of the tokens list to create the AST out of them.
...
4. **Domain Calculator:** Alot of mathematical but fun stuff here. We first recurse through the AST and create a constraints list for each set constraint we encounter. After that we find critical points of the constriant's expression. We then test the intervals generated by checking if they satisfy the constraint. Finally once we have the intervals in the form of (lo, hi, lo_incl, hi_incl) we can simply union and intersect the required intervals to find the final domain. The main fun stuff lies in how the criitical points are found. We basically use a brute force method by using a window of points to evaluate the function at by substituting in the value of x. We note a critical point by either a sign flip or definition change. After we have the rough idea of where the point lies, we send that to the bisect function which does the same thing but keeps on shortening the window so that it could "zoom into" the value more and get the value to a more precise value. Now there are of course a lot of edge cases to this approach like the fixed window size and more and I do think that changing the approach to something like Newton-Raphson's algorithm would be better. But, I will keep that update for the future because that also comes with its own complicated edge cases.